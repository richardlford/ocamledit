...
(* 
  Copy a bytecode executable or other binary, performing requested
  edits such as updating the debug information (or deleting it).
*)
...
let usage =
"Usage: ocamledit [options] <input file> <output file>\n\
Options are:"

let main() =
  printf "@.Greetings from ocamledit!@.";
  let anon = ref [] in
